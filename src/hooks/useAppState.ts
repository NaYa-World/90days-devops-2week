...
export function useAppState() {
  const [currentUser, setCurrentUser] = useState<string | null>(() => {
    return localStorage.getItem('devops90_current_user');
  });

  const userKey = currentUser ? `${LOCAL_STORAGE_KEY_PREFIX}${currentUser.toLowerCase()}` : 'devops90_v4_guest';

  const [state, setState] = useState<AppState>(() => {
    const user = localStorage.getItem('devops90_current_user');
    if (user) {
  ...
  const saveStateToStorage = (updated: AppState, user: string | null) => {
    if (!user) return;
    try {
  ...
  const loginUser = (username: string, password?: string): boolean => {
    ...
    if (match) {
      ...
      if (pendingCount > 0) {
        const alertMsg = `📝 Reminder: You have ${pendingCount} pending tasks remaining. Let's finish them!`;
        ...
        if (!exists) {
          ...
          try {
            ...
            localStorage.setItem(key, JSON.stringify(rawStored));
          } catch (_) {}
        }
      }
  ...
  const addNotification = (text: string) => {
    updateState(prev => {
      ...
      return {
        ...prev,
        notifications: [newNotif, ...list].slice(0, 30)
      };
    });
    showToast(text);
  };
  ...
  const toggleTask = (pi: number, di: number, ti: number) => {
    ...
    const taskName = task ? task.t : 'Task';
    ...
    updateState(prev => {
      ...
      if (wasDone) {
        ...
        nextNotifs = [newNotif, ...list].slice(0, 30);
      } else {
        // Task completed
        const newNotif = {
          id: Date.now(),
