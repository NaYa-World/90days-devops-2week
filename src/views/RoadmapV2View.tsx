...
function saveV2State(key: string, s: Record<string, boolean>) {
...
export const RoadmapV2View: React.FC<Props> = ({ appState }) => {
  ...
  React.useEffect(() => {
    const active = localStorage.getItem('devops90_v2_active_day');
    if (active) {
      ...
      if (parts.length === 2 && !isNaN(parts[0]) && !isNaN(parts[1])) {
        ...
        localStorage.removeItem('devops90_v2_active_day');
  ...
  function bulkMarkDayV2(pi: number, di: number, type: 'concept' | 'code' | 'all') {
    const next = { ...v2state };
    const tasks = PHASES_V2[pi]?.data[di]?.tasks || [];
    tasks.forEach((task, ti) => {
      if (type === 'all' || task.k === type) {
        next[v2key(pi, di, ti)] = true;
      }
    });
    setV2State(next);
    saveV2State(userKey, next);
                        ...
                        <div style={{ overflow: 'hidden' }}>
                          <div style={{ padding: '0 20px 14px 26px' }}>
                            ...
                            <div className="bulk-actions" style={{ marginBottom: '8px' }}>
                              <button className="bulk-btn" onClick={(e) => { e.stopPropagation(); bulkMarkDayV2(pi, di, 'concept'); }}>
                                ✓ All Concepts
                              </button>
                              <button className="bulk-btn" onClick={(e) => { e.stopPropagation(); bulkMarkDayV2(pi, di, 'code'); }}>
                                ✓ All Code
                              </button>
                              <button className="bulk-btn" style={{ color: 'var(--green)' }} onClick={(e) => { e.stopPropagation(); bulkMarkDayV2(pi, di, 'all'); }}>
                                ✓ All Tasks
                              </button>
                            </div>
