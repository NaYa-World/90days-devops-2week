...
export const ResumeView: React.FC<ResumeViewProps> = ({ appState }) => {
  ...
  const handleGenerate = () => {
    ...
    const finalName = name.trim() || 'GK';
    const finalEmail = email.trim() || 'gk@gmail.com';
    const finalPhone = phone.trim() || '+91 9999999999';
    const finalLoc = loc.trim() || 'Hyderabad, India';
    const finalLinkedin = linkedin.trim() || 'linkedin.com/in/yourname';
    const finalGithub = github.trim() || 'yourusername';

    let resume = `${finalName.toUpperCase()}\n`;
    resume += `${finalEmail}${finalPhone ? ' | ' + finalPhone : ''}${finalLoc ? ' | ' + finalLoc : ''}\n`;
    resume += `${finalLinkedin ? 'LinkedIn: ' + finalLinkedin + ' ' : ''}${finalGithub ? 'GitHub: ' + finalGithub : ''}\n`;
    resume += `${'─'.repeat(60)}\n\n`;
    ...
    if (builtProjects.length > 0) {
      builtProjects.forEach(proj => {
        ...
        resume += `  github.com/${finalGithub}/${proj.repoName}\n\n`;
      });
    ...
    <div className="wrap">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '12px' }}>
        ...
        <div>
          ...
          <input className="v4-input" value={email} onChange={e => setEmail(e.target.value)} placeholder="gk@gmail.com" />
        </div>
        <div>
